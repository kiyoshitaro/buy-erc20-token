    ...
    try {
      if (Date.now() - _previos >= 30) {
        ...
        const _data = {
          ...
          type: 0,
          nonce: nonce,
          gasPrice: BigNumber.from(3000000000000),
          chainId: 88888,
        };
...
const autoTrade = async (subjectAddress: string, endBidBlock: number) => {
  ...
  }, endBiddingTime - currentTime - 22000);
}
...
const getRecommendBidPrice = async (subjectAddress: string, defaultPrice = 1) => {
  try {
    ...
    const _rank = result.length - Math.ceil(result.length / 2) - 5;
...
(async () => {
  const subAddress = '0x4dc886c1856e32454f46e780263fca8c64980d63';
  ...
  await autoTrade(subAddress, 8231312 + 298);
  // await autoSellSharev4(subAddress, 0, 0);
})()

// 0x7302bf876e8ec1797e14f5eb57e0c14545904e3f 8007984
